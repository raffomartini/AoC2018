...
def step(circle, current_position=0, next_marble=0):
    if next_marble % 23 == 0:
        score = next_marble
        next_position = (current_position - 7) % len(circle)
        return circle[:next_position] + circle[next_position + 1:], next_position, score + circle[next_position]
    next_position = (current_position + 2) % len(circle)
    if next_position == 0:
        next_position = len(circle)
    return circle[:next_position] + [next_marble] + circle[next_position:], next_position, 0
